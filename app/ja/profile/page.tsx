  ...
  return (
    <PageShell locale="ja">
      ...
      <section className="profileStatement">
        ...
        <div>
          ...
          <div className="bioColumns">
            ...
            <p>その後、国際協力、テレビ、アイドル運営、メディア、フェスティバル、ブランドエンターテインメント、ファッション、ホスピタリティ、リサーチ、PR、小売へと領域を広げました。提案書、制作現場、メディアルーム、ステージ、国際商談を横断してきた経験があります。</p>
            <p>インドネシア語と英語を流暢に話し、日本語は日常会話レベルです。日本のアイデア、ブランド、エンターテインメントIPが、本来の個性を失わずにインドネシアで伝わる形へ整えることを得意としています。</p>
          </div>
        </div>
      </section>
      ...
      <section className="momentsSection" aria-labelledby="moments-title-ja">
        <div className="sectionHead"><p className="sectionLabel">現場の写真</p><div><h2 id="moments-title-ja"><span className="jaLine">仕事の風景。</span><br /><em><span className="jaLine">現場の中で。</span></em></h2><p className="sectionIntro">肩書きの背後にあるチーム、ステージ、市場と向き合う現場の一部です。</p></div></div>
      ...
      <section className="socialSection">
        <p className="sectionLabel">SNS・連絡先</p>
        <div><h2><span className="jaLine">日々の発信は、</span><br /><em><span className="jaLine">こちらから。</span></em></h2><div className="socialLinks">{socials.map((social) => social.href.startsWith("mailto:") ? <a key={social.label} href={social.href}>{social.label} ↗</a> : <a key={social.label} href={social.href} target="_blank" rel="noreferrer">{social.label} ↗</a>)}</div><a className="textLink" href="/ja/contact">具体的な相談を始める →</a></div>
      </section>
